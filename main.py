...
from aiogram.types import Message, CallbackQuery, LabeledPrice, Location
...
async def command_start(message: Message):
    await message.answer(f'Здраствуйте {message.from_user.full_name}.Вас приветствует бот Shop ')
...
async def register_user(message: Message):
    ...
    user = first_select_user(cha_id)
    if user:
        await message.answer('Авторизация прошла успешно')
        await show_main_menu(message)
    else:
        first_register_user(cha_id, full_name)
...
# ----------------------------------------------------------------------
@db.message_handler(regexp=r'✔ Сделать заказ')
...
async def show_detail_product(call: CallbackQuery):
    ...
    product = get_proguct_detail(product_id)
...
# Функция для добавления товара на единицу
...
async def deleter_cart_products(call: CallbackQuery):
...
async def get_payment(message:Message):
    chat_id = message.chat.id
    cart_id = get_user_cart_id(chat_id)
    await bot.send_message(chat_id, 'Ваша оплата прошла успешно. Ожидайте звонок')
